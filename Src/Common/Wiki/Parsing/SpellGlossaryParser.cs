        ...
        public void Parse(WikiPage glossaryPage, List<Spell> spells)
        {
            ...
            foreach (var spell in spells)
            {
                ...
                else
                {
                    ...
                    this.log.Warning("Le sort {0} ({1} {2}) n'est pas présent dans le glossaire anglais/français. Son id sera généré en français.", spell.Name, spell.Id, spell.Source.Id);
                }
            }

            foreach (var pair in dict)
            {
                this.log.Warning("Le sort {0} ({1}) du glossaire n'a pas été utilisé", pair.Value, pair.Key);
            }
        }
